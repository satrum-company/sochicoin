...
uint256 hashGenesisBlock("0x5f825a66c8f2bcc8db57a3664adddf6b2809585a50d686cbf58d6941e517e708");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sochicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sochicoin Signed Message:\n";
...
{
    // Sochicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 294 * COIN;
    std::string cseed_str = prevHash.ToString().substr(8,7);
	const char* cseed = cseed_str.c_str();
	long seed = hex2long(cseed);
	int rand = generateMTRandom(seed, 100000);	

	if (((rand > 20000) && (rand < 22274)) || ((rand > 70000) && (rand < 72274)))
	{
		// Bonus 1 (4.545%)
		nSubsidy = 588 * COIN;
	} else if (((rand > 30000) && (rand < 30170)) || ((rand > 80000) && (rand < 80170))) {
		// Bonus 2 (0.34%)
		nSubsidy = 1176 * COIN;
	} else if (((rand > 10000) && (rand < 10025)) || ((rand > 90000) && (rand < 90025))) {
		// Bonus 3 (0.04965%)
		nSubsidy = 2352 * COIN;
	}
	
	// halved every 2 years, including the bonus too
	nSubsidy >>= (nHeight / 353454);
		
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60; // SochiCoin: 1 Hour
static const int64 nTargetSpacing = 88; // SochiCoin: 88 seconds
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4f20f0ce6d6fda09daf9f51aed6b25c35419655f951da66980677c590f5e4a5a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "SOCHICoin 02/07/2014 In Mean, SOCHI Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1391621425;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2092893539;
        ...
        assert(block.hashMerkleRoot == uint256("0xef3fb3d3363720b6fafeefb8e9c41b6ed5fd20feef3b2d0fa9900ada5c687098"));
...
unsigned char pchMessageStart[4] = { 0xae, 0xa7, 0xf2, 0xd1 }; // Litecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SochicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SochicoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("SochicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sochicoin-miner");
    ...
    try { while(true) {
        ...
        printf("Running SochicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SochicoinMiner terminated\n");
        throw;
    }
}
